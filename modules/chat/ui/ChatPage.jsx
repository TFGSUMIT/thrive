...
// the full open-webui (migrated to core) — the "power" chat with history/RAG
const BRAIN_URL = 'https://brain.nerfarrow.com'
...
export default function ChatPage() {
  ...
  // ── styles ──
  // Pin to the viewport below the 48px nav (fixed, not 100vh math) so the composer
  // is ALWAYS on-screen regardless of UI zoom / page flow.
  const outer = { position: 'fixed', top: 48, left: 0, right: 0, bottom: 0, display: 'flex', justifyContent: 'center' }
  const wrap  = { width: '100%', maxWidth: 860, display: 'flex', flexDirection: 'column', height: '100%' }
  const bar  = { display: 'flex', gap: 10, alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid var(--border-color,#2a2a2a)', flexShrink: 0 }
  ...
  return (
    <div style={outer}><div style={wrap}>
      <div style={bar}>
        ...
        <div style={{ flex: 1 }} />
        <a href={BRAIN_URL} target="_blank" rel="noopener noreferrer" title="Open the full Open WebUI (history, RAG, tools)"
          style={{ ...btnS, textDecoration: 'none' }}>Full ↗</a>
      ...
      <div style={{ display: 'flex', gap: 10, padding: '12px 16px', borderTop: '1px solid var(--border-color,#2a2a2a)', alignItems: 'flex-end', flexShrink: 0 }}>
        <textarea
